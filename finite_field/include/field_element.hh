...
using namespace std;
/**
  Defines a Galois Field element. Also defines arithmetic operations on Galois
  Field elements.

  In mathematics, an element of the Galois Field GF(p^k) is a polynomial of
  degree less than k with coefficients chosen from the set {0,1,2,..., p-1}.
  ...
  @param polynomial the array representing the polynomial describing this
  FieldElement
  */
class FieldElement {
public:
  ...
  /**
    ...
    @param gf pointer to a Galois Field
    @param poly an external array containing coefficients (length of array =
    dimension)
    */
  ...
  /**
    ...
    It sets all coefficients of FieldElement polynomial to zero except the
    zero-th degree coefficient which is set to 1.
    */
  auto set_one(void) -> void;

  /**
    Sets the i-th coefficient of FieldElement's polynomial to a specified
    constant
  ...
  /**
    Sets all coefficients of FieldElement's polynomial to the same specified
    constant
  ...
  /**
    ...
    This can be used as an l-value. It sets the i-th coefficient to a specified
    value.
    @param i (input), index
  ...
  /**
    ...
    */
  friend FieldElement operator+(const FieldElement &left,
  ...
  /**
    ...
    It uses the repeated square-and-multiply algorithm for exponentiation in
    GF(p^k).
