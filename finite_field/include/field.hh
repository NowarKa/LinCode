...
/**
   This class defines a Galois Field of characteristic p and dimesion k (for
   GF(p^k)). It stores the following (private) data about the field:
   @param the characteristics p, which MUST be a prime number.
   @param dimension The dimension (k) of Galois Field (a nonzero positive
   integer).
   @param order The order (p^k) of Galois Field.
   @param reductpoly A polynomial of degree k over the prime field GF(p). This
   polynomial MUST be irreducible. The program does not check this. YOU MUST
   correctly choose this polynomial.
...
public:
  ...
  /**
    ...
    */
  auto get_element(uint64_t i) const -> FieldElement;
  ...
  friend bool operator==(const Field &left, const Field &right) {
    return left.order == right.order && left.reductpoly == right.reductpoly;
  };
...
inline auto Field::get_dimension() const -> uint32_t { return dimension; }

inline auto Field::get_order() const -> uint64_t { return order; }

inline Fint Field::reduction_polynomial(int i) const { return reductpoly[i]; }
