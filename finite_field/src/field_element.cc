...
FieldElement::FieldElement(const std::shared_ptr<const Field> gf)
    : field(gf), modulus(field->get_characteristic()),
      dimension(field->get_dimension()), polynomial(vector<Fint>(dimension)) {}

/* ************************************************************************* */
FieldElement::FieldElement(const std::shared_ptr<const Field> &gf,
                           const vector<Fint> coeff)
    : field(gf), modulus(field->get_characteristic()),
      dimension(field->get_dimension()), polynomial(dimension, 0) {
...
/* ************************************************************************* */
Fint &FieldElement::operator[](int index) { return polynomial[index]; }

Fint FieldElement::operator[](int index) const { return polynomial[index]; }
...
FieldElement operator+(const FieldElement &left, const FieldElement &right) {
  ...
  for (int i = 0; i < min(left.polynomial.size(), right.polynomial.size()); i++)
    result.polynomial[i] =
        add_modular(left.polynomial[i], right.polynomial[i], left.modulus);
  return result;
}
...
FieldElement operator-(const FieldElement &left, const FieldElement &right) {
  ...
  for (int i = 0; i < min(left.polynomial.size(), right.polynomial.size()); i++)
    result.polynomial[i] =
        subtract_modular(left.polynomial[i], right.polynomial[i], left.modulus);
  return result;
}
...
FieldElement operator*(const FieldElement &left, const FieldElement &right) {
  ...
  auto product =
      polynomial_multiply(left.polynomial, right.polynomial, modulus);
...
FieldElement operator*(Fint left, const FieldElement &right) {
  ...
  for (int i = 0; i < right.dimension; i++)
    result.polynomial[i] =
        multiply_modular(left, right.polynomial[i], right.modulus);
  return result;
}
...
FieldElement operator*(const FieldElement &left, Fint right) {
  ...
  for (int i = 0; i < left.dimension; i++)
    result.polynomial[i] =
        multiply_modular(right, left.polynomial[i], left.modulus);
  return result;
}
...
FieldElement operator/(const FieldElement &left, Fint right) {
  ...
  for (int i = 0; i < left.dimension; i++)
    result.polynomial[i] =
        divide_modular(left.polynomial[i], right, left.modulus);
  return result;
}
...
FieldElement mult_inverse(const FieldElement &FieldE) {
  ...
  auto [dummy, inversepoly, gcd] = polynomial_extended_euclid(
      reductionpolynomial, FieldE.polynomial, modulus);
